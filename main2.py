...
class FaceRecognitionApp:
    def __init__(self, root, video_source=0, resolution=(320, 240), fps=15):
        ...
        # Load reference images from the "caras" directory
        ...
        # Using threading for showing frame
        ...
        self.start_stop_script_button = tk.Button(self.root, text="Parar Programa", command=self.stop)
    ...
    def load_reference_encodings(self, directory):
        reference_encodings = []
        ...
        while not self.stop_script:
            ...
            if face_locations:
                ...
                for face_location, face_encoding in zip(face_locations, face_encodings):
                    ...
                    # Compare face encoding with all reference encodings
                    for i, reference_encoding in enumerate(self.reference_encodings):
                        ...
                        if results[0]:
                            ...
                            break  # Exit loop if face is recognized
    ...
    def add_person(self):
        ...
        # Create a button for uploading a photo
    ...
    def upload_photo(self):
        ...
        try:
            ...
            if self.file_path:
                ...
                image = image.resize((300, 300), Image.Resampling.LANCZOS)  # Resize the image to fit within a 300x300 area

                # Convert the image to a format that tkinter can display
                photo = ImageTk.PhotoImage(image)

                # If there is an existing image label, destroy it
                ...
                name_label = tk.Label(self.ventana_foto, text="Nombre:")
                name_label.pack()
                self.name_entry = tk.Entry(self.ventana_foto)
                self.name_entry.pack()

                age_label = tk.Label(self.ventana_foto, text="Edad:")
                age_label.pack()
                ...
                self.ventana_foto.geometry("400x600")

                self.ventana_foto.focus_set()
            else:
                print("No file selected.")
        except Exception as e:
            logging.error(f"Error al cargar la foto: {e}")
    ...
    def submit_photo(self):
        ...
        if not name or not age:
            print('No se lleno el campo nombre o edad')
        ...
        try:
            ...
            # Mover la imagen del usuario al directorio "caras" con el ID como nombre de archivo
            image_filename = user_id + "ori.jpg"
            image_destination = join("carasori", image_filename)
            shutil.copy(self.file_path, image_destination)

            print("Imagen Subida")
            self.ventana_foto.destroy()

            self.reference_encodings = self.load_reference_encodings("caras")
        except Exception as e:
            logging.error(f"Error al agregar persona: {e}")
        ...
        try:
            ...
            if face_locations:
                ...
                for i, face_location in enumerate(face_locations):
                    ...
                    # Recortar el área de la cara de la imagen
            ...
            logging.error(f"Error al recortar caras: {e}")
